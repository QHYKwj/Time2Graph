...
# def filter_invalid_samples2(X, Y):
    """
    X: (N, L, D)
    Y: (N,)
    过滤掉任何在 X 或 Y 中包含 NaN/inf 的样本
    """
    # X 中每个样本是否全部是有限值
    mask_X = np.isfinite(X).all(axis=(1, 2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--transformer_ff', type=int, default=256, help='hidden dimension in Transformer FFN')
    parser.add_argument('--dropout', type=float, default=0.1, help='dropout rate')
    parser.add_argument('--lr', type=float, default=1e-3, help='learning rate')
    parser.add_argument('--num_epochs', type=int, default=50, help='number of training epochs')
    parser.add_argument('--regressor_batch_size', type=int, default=64, help='batch size for regressor training')
    ...
    # 遍历风场1和风场2中的所有风机，训练并保存每个风机的模型
    farms = ['风场1', '风场2']
    turbine_ids = {
        '风场1': [f"m{str(i).zfill(2)}" for i in range(1, 26)],  # m01 到 m25
        '风场2': [f"m{str(i).zfill(2)}" for i in range(26, 51)]  # m26 到 m50
    }

    for farm in farms:
        for turbine_id in turbine_ids[farm]:
            print(f"正在训练 {turbine_id} 的模型...")
            
            # 获取风机的训练数据和测试数据
            x_train_seq, y_train_seq, x_test_seq, y_test_seq = load_full_dataset(base_dir, farm, turbine_id, train_end_date)
            # 2) 切成 Time2Graph 需要的 (N, L, D)
            # 构建 Time2GraphWindModel 需要的数据格式
            ...
            # 在送进 Time2Graph 前，先过滤 NaN/inf
            X_train, Y_train = filter_invalid_samples(X_train, Y_train)
            X_test, Y_test   = filter_invalid_samples(X_test, Y_test)
            print(f"{farm}/{turbine_id} -> X_train: {X_train.shape}, Y_train: {Y_train.shape}, " f"X_test: {X_test.shape}, Y_test: {Y_test.shape}")
            # 创建模型
            ...
            if args.cache:
                m.save_model(fpath='{}/scripts/cache/{}/{}_embedding_t2g_model.cache'.format(module_path,farm,turbine_id))
            ...
            # 打印结果
            Debugger.info_print(f"{turbine_id} 的结果: "
