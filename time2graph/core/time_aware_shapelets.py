...
def parameterized_gw_torch(x, y, w, torch_dtype, warp=2):
    ...
    #                           axis=1)
    return torch.sqrt(torch.sum(torch.from_numpy(distance).type(torch_dtype) * torch.abs(w)))
...
def pattern_distance_torch(pattern, time_series, num_segment, seg_length,
    ...
    # 直接返回距离的加权和，不使用softmax
    return torch.sum(distance * torch.abs(global_factor))
...
def __shapelet_candidate_loss(cand, time_series_set, label, num_segment, seg_length,
    ...
    while cnt < max_iters:
        ...
        for x, y in dataloader:
            ...
            # 使用均方误差(MSE)来计算损失
            main_loss = torch.mean((dist_tensor - y) ** 2)  # MSE
            penalty_loss = (alpha * torch.norm(local_factor_variable, p=p) / seg_length) + \
                           (beta * torch.norm(global_factor_variable, p=p) / num_segment)
